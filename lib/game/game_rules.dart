...
class GameRules {
  ...
  bool isValidMove(Position from, Position to, PlayerColor currentPlayer) {
    ...
    if (board.isOwnDen(to, currentPlayer)) {
      print("Invalid: moving to own den");
      return false;
    }

    Piece? targetPiece = board.getPiece(to);
    print("Target piece at to: $targetPiece");

    // Can't capture own pieces
    if (targetPiece != null && targetPiece.playerColor == currentPlayer) {
      print("Invalid: capturing own piece");
      return false;
    }

    // Check if it's a jump move
    if (_canJumpOverRiver(from, to, piece)) {
      print("Is a jump move");
      // If it's a jump, the 'to' position must be empty or contain a capturable piece
      if (targetPiece == null) {
        print("Valid jump: target empty");
        return true;
      } else {
        print("Valid jump: target capturable");
    ...
    if (targetPiece != null) {
      print("Target piece exists, checking capture rules");
      // Pieces in traps can be captured by any opponent piece
      if (board.isTrap(to)) {
        print("Target in trap, checking opponent piece");
      ...
      if (gameConfig.ratCannotCaptureElephant &&
        ...
        print("Invalid: rat cannot capture elephant variant");
        return false;
      }

      // Normal capture rules
      print("Normal capture rules");
      return piece.canCapture(targetPiece);
    }

    print("Valid adjacent move");
    return true;
  }
  ...
  bool _canJumpOverRiver(Position from, Position to, Piece piece) {
    ...
    if (!canJump) {
      print("Cannot jump: piece type not allowed");
      return false;
    }
    ...
    if (from.row == to.row) {
      ...
      print("Horizontal jump: distance=$distance");
      ...
      if (gameConfig.extendedLionTigerJumps &&
        ...
        print("Double river cells: $river1_1, $river1_2, $river2_1, $river2_2");
        if (board.isRiver(river1_1) &&
          ...
          if (board.getPiece(river1_1)?.animalType == AnimalType.rat ||
            ...
            print("Cannot jump: rat in double river");
            return false;
          }
          print("Valid double horizontal jump");
          return true;
        }
      }
      ...
      if (gameConfig.extendedLionTigerJumps &&
        ...
        print("Leopard river cells: $river1, $river2");
        if (board.isRiver(river1) && board.isRiver(river2)) {
          Piece? rat1 = board.getPiece(river1);
          Piece? rat2 = board.getPiece(river2);
          print("Rats in leopard river: $rat1, $rat2");
          if ((rat1 != null && rat1.animalType == AnimalType.rat) ||
              (rat2 != null && rat2.animalType == AnimalType.rat)) {
            print("Cannot jump: rat in leopard river");
            return false;
          }
          print("Valid leopard horizontal jump");
          return true;
        }
      }
    }
    // Check vertical jump (up or down)
      ...
      print("Vertical jump: distance=$distance");
      ...
      if ((piece.animalType == AnimalType.lion ||
        ...
        print("River cells: $river1, $river2, $river3");
        if (board.isRiver(river1) &&
          ...
          print("Rats in river: $rat1, $rat2, $rat3");
          if ((rat1 != null && rat1.animalType == AnimalType.rat) ||
              (rat2 != null && rat2.animalType == AnimalType.rat) ||
      ...
      if (gameConfig.extendedLionTigerJumps &&
        ...
        if (board.isRiver(river1_1) &&
          ...
          print("Valid double vertical jump");
          return true;
        }
      }
    }

    print("Invalid jump: no specific rule matched");
    return false;
  }
}
